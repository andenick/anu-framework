...
Scrub patterns (FAIL-severity): `<workspace>`, `/Council/`, `\bDruck\b`, `\bRobin/`. Scrub patterns (WARN-severity): `DEC-[A-Z0-9]+` (internal decision codes inherited from predecessors).
...
| P11_NO_ARCANUM_REFS | WARN | No references to Arcanum, <internal-tool>, freenic, Robin |
