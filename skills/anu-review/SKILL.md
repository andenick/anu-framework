...
**Why this matters**: In the reference implementation, automated validation (V01: 383/383 PASS) missed an 88x unit mismatch (S069) and a 5340x scale mismatch (S087) that were only caught by reading the actual data against the book. Source material reading is not optional.
...
## File Locations (Example: the reference implementation)

> **Note**: The paths below are from the reference implementation. Other projects will have the same structure under their own `Technical/` directory.
