# Anu Framework Improvements RFC — From Reference-Replication Build Experience
...
- Run `anu-ingestion migrate-scheme --table mapping.csv --dry-run` on the reference replication with the T→S/N→ES table. Should report changes that match `_build_registry.py`'s output byte-for-byte.
...
- Run `anu-ingestion batch-create-dpr --cohort wave_1_ch5` on the reference replication. Output should match (modulo my hand-edits) the existing `docs/series/S5*_DPR.md` files generated by my `_gen_dprs.py`.
...
- First run on the reference replication: 152 findings (mostly `<decision-ref>` inherited from ST2). After scrub: 0 findings on 653 files.
...
- Run `anu-publish audit` on the reference replication. Output identical to my S06 final run: `CLEAN — 653 files, zero internal references.`
...
```python
# anu-replicator/templates/lib/data/book_table.py (extracted from the reference replication's code/utils/io.py)
def read_book_table(path: Path) -> pd.DataFrame:
    ...
    """
    # (current logic from the reference replication's code/utils/io.py:read_book_table)
```
...
- `anu-research/SKILL.md` assumes de-novo synthesis from KB extractions (PDF-extraction outputs). No port command.
...
def run():
    df = LOADER.load()
    # … (same shape as my reference-replication loaders)
```
...
- `anu-pipeline init` in a new project creates `run.py`. Running `python run.py --validate-only` on the reference replication post-scaffold should reproduce the 64 PASS result.
...
```python
# anu-architecture/templates/lib/data/bea_cache.py (new — direct port of the reference replication's code/utils/bea_cache.py)
...
- The 21-commit reference-replication rebuild was structured as 6 waves + 4 sprints — a cadence I invented mid-build (documented retroactively in `docs/ROADMAP.md` and `docs/IMPLEMENTATION_PLAN.md`).
...
- Running `anu-doctor project` on the reference replication post-Wave-6 should report all 10 checks CLEAN.
...
- **Phase E** — Validation: apply enhanced framework to a new rebuild target (e.g., porting another economic data project) and measure session count vs. reference-replication baseline of 12-15 sessions. Target: 7-10 sessions.
...
- `anu-doctor project` runs against the reference replication and reports CLEAN.
