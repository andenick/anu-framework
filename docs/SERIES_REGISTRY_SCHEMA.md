...
The reference implementation `<project>/Technical/series_registry.json` is the canonical sample-of-truth and is expected to validate cleanly. When this spec and the schema disagree with the CD2 registry, the registry wins until the next dated revision of this document.
