...
## Data Repository Integration (across all stages)

A **canonical data repository** (a versioned, checkout-based store of economic
source data) can serve as the framework's primary external data source. Where one
is configured, integration touches every stage:

| Stage | How it touches the data repository |
|---|---|
| **Stage 1 — anu-research** | Research JSONs cite repository sources by `data_source_id` (matches `AUTHORITATIVE_COUNTS.json` keys). |
| **Stage 2 — anu-adequacy** | +1 score if a cited source is in the repository and live; -1 if cited but absent from the repository and the open-API set. |
| **Stage 2.5 — Data Acquisition** | Project creates `inputs/data-repository/[SOURCE]/` checkouts per the data-checkout contract. PROVENANCE.md records `data_source_id`, version, file hashes. |
| **Stage 3 — anu-ingestion** | L## loaders import `lib/data/data_loader.py`. Never read the repository's canonical store directly. |
| **Stage 4 — anu-extension** | EPRs record the repository source + snapshot timestamp. Extension refreshes the checkout when the canonical updates. |
| **Stage 5 — anu-replicator** | `lib/data/data_loader.py` is the canonical helper. Hash-stamps every load against the repository version. |
| **Stage 6 — anu-chopped / anu-extenbook** | Output formats record `data_source_id` in subsource metadata. |
| **Stage 7 — anu-review** | D13 Data Authenticity gate checks: every repository-sourced series has valid PROVENANCE + non-drifting hashes. |
| **Stage 8 — anu-publish / anu-archive** | Published manifests include the `data_repo_version` pinned at construction time. |
...
- Hardcoded paths into the repository's canonical store
...
- **The example replication** (`projects/example-replication/`) — replication of Anwar Shaikh's *Capitalism: Competition, Conflict, Crises* (2016). Most complete project; exercises all skills. The v11.0 reference.
- **The reference replication** (`projects/reference-replication/`) — replication of Shaikh & Tonak's *Measuring the Wealth of Nations* (1994). 64 series, first project built end-to-end on anu-build v12.0.

---

## Paper Target Ledger (Replication Contract Phase)

**Added 2026-05-27 (additive).**
...
`projects/<project>/Technical/studies/<study>/code/lib/target_ledger.py`
(study-local; promotion to a shared framework module pending maintainer approval).
...
*Anu Framework v12.2 — Data Construction, Empirical Research, and Reproducible Publication*
*Location: docs/ANU_FRAMEWORK_OVERVIEW.md*
